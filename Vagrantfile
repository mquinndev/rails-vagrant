Vagrant.configure("2") do |config|
  ...
  config.vm.provision :shell, path: "bin/python_install.sh", privileged: false
  config.vm.provision :shell, path: "bin/ruby_install.sh", args: "2.4.0 rails", privileged: false
  config.vm.provision :shell, path: "bin/docker_install.sh"
  config.vm.provision :shell, path: "bin/rvm_install.sh", args: "stable", privileged: false
